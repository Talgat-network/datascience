...
new_name = ['user_id','track_name','artist_name','genre_name','city','time','weekday'] # <переименование столбцов>
...
def find_genre(genre_name): # <создание функции find_genre()>
    ...
    result = 0 # в теле объявляется переменная-счётчик, ей присваивается значение 0,
    for row in genres_list: # затем цикл for проходит по списку уникальных значений
        if row == genre_name: # если очередной элемент списка равен параметру функции, 
            result +=1 # то значение счётчика увеличивается на 1
    return result # по окончании работы цикла функция возвращает значение счётчика
...
def find_hip_hop(df, wrong): # <создание функции find_hip_hop()>
    df['genre_name'] = df['genre_name'].replace(wrong,'hihop')
    genre_count = df[df['genre_name'] == wrong]
    genre = genre_count.loc[:,'genre_name']                                # функция принимает как параметры таблицу df и неверное название
    genre_final_count = genre.loc[genre == wrong].count()      # к столбцу 'genre_name' применяется специальный метод, 
    return genre_final_count                           # который заменяет второй параметр на строку 'hiphop'
...
def number_tracks(df,day,city):    # <создание функции number_tracks()>
    track_list = df[(df['weekday'] == day) & (df['city'] == city)]                  # объявляется функция с тремя параметрами: df, day, city
    track_list_count = track_list.loc[:,'genre_name'].count()
...
table = pd.DataFrame(data = data, columns =  ['city', 'monday', 'wednesday', 'friday'] ) # <таблица с полученными данными>
table
...
def genre_weekday(df,day,time1,time2): # объявление функции genre_weekday() с параметрами df, day, time1, time2
    genre_list = df[(df['weekday'] == day) & (df['time'] > time1) & (time2 > df['time'])]    # в переменной genre_list сохраняются те строки df, для которых одновременно:
